...
Private Sub cmdClasses_Click()
    ...
    Log "[7] LoadAll() â€” read first 5 big rows into 2D array"
    ...
    Log "    db variable set to Nothing â€” implicit close happened"
    Log ""
    Log "=== done ==="
...
Private Sub cmdMaint_Click()
    ...
    ' Build a chunky table â€” 5000 rows with ~200 bytes of text each.
    ...
    ' --- VACUUM ---------------------------------------------------
    Log "[3] CompactDB (VACUUM) â€” rebuilding the file..."
    Dim t0 As Single
    t0 = Timer
    Dim saved As Double
    saved = sch.CompactDB
